...
func (this *Message) EqualVT(that *Message) bool {
	...
	if string(this.Comment) != string(that.Comment) {
		return false
	}
	if this.Corpus != that.Corpus {
		return false
	}
...
func (m *Message) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Corpus != 0 {
		i = encodeVarint(dAtA, i, uint64(m.Corpus))
		i--
		dAtA[i] = 0x28
	}
	if len(m.Comment) > 0 {
	...
	if m.ResultPerPage != 0 {
		i = encodeVarint(dAtA, i, uint64(m.ResultPerPage))
		i--
		dAtA[i] = 0x18
	}
	if m.PageNumber != 0 {
		i = encodeVarint(dAtA, i, uint64(m.PageNumber))
		i--
		dAtA[i] = 0x10
	}
	if len(m.Query) > 0 {
...
func (m *Message) SizeVT() (n int) {
	...
	if m.PageNumber != 0 {
		n += 1 + sov(uint64(m.PageNumber))
	}
	if m.ResultPerPage != 0 {
		n += 1 + sov(uint64(m.ResultPerPage))
	}
	l = len(m.Comment)
	if l > 0 {
		n += 1 + l + sov(uint64(l))
